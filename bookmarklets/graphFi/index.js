  ...
  function draw() {
    ...
    for (var i = 0; i < maxCommentsAtOnce; i++) {
      ...
      if (selecting || hovering) {
        ...
        cIndex >= minCommentInViewport &&
        cIndex < maxCommentInViewport
      ) {
      ...
      for (var j = 0; j < quotes.length; j++) {
        ...
        if (q != i) {
          ...
          ctx.quadraticCurveTo(
            cWidth * 0.5 - cWidth * ((i - q + 1) / comments.length),
  ...
  function setUpInteraction() {
    ...
    canvas.bind('mousemove.graphfi', function(evt) {
      ...
      if (newhovered != hovered) {
        ...
        var tooltipY;
        if (
          y > canvasHolder.height() / 2 ||
          y + tooltip.height() > canvasHolder.height()
        ) {
      ...
      if (maxCommentsAtOnce < comments.length) {
        if (minCommentInViewport > 0) {
          var pos = Math.max(
            0,
            Math.min(
              comments.length - maxCommentsAtOnce - 1,
